...
checkingAndDownloadingFreeType()
{
  ...
  if [[ ! -z "$FOUND_FREETYPE" ]] ; then
    ...
    if ! (bash ./configure "${BUILD_CONFIG[FREETYPE_FONT_BUILD_TYPE_PARAM]}" --host=x86_64-w64-mingw32 && ${BUILD_CONFIG[MAKE_COMMAND_NAME]} setup --host=x86_64-w64-mingw32 && ${BUILD_CONFIG[MAKE_COMMAND_NAME]}); then
